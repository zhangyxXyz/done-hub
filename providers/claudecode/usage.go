...
func (e usageWindowUnavailableError) Error() string {
	return e.message
}

func isUsageWindowUnavailable(err error) bool {
	_, ok := err.(usageWindowUnavailableError)
	return ok
}
...
func (p *ClaudeCodeProvider) RequestUsage() (UsageStatus, int, error) {
	...
	if usage == nil {
		return UsageStatus{}, statusCode, usageWindowUnavailableError{message: "ClaudeCode usage response is empty; no active usage window may have been created yet"}
	}
	if _, ok := usage["five_hour"]; !ok {
		if _, ok := usage["seven_day"]; !ok {
			body, _ := json.Marshal(usage)
			return usage, statusCode, usageWindowUnavailableError{message: fmt.Sprintf("ClaudeCode usage response has no utilization windows yet: %s", string(body))}
		}
	}
...
func (p *ClaudeCodeProvider) RequestUsageWithCache() (*UsageResult, error) {
	cacheConfig := p.GetUsageCacheConfig()
	if cacheConfig.TTLSeconds <= 0 {
		usage, statusCode, err := p.RequestUsage()
		if err != nil {
			if isUsageWindowUnavailable(err) {
				return &UsageResult{
					Usage:      usage,
